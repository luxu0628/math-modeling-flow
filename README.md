...
[![Bundle](https://img.shields.io/badge/Skills-12个开箱即用-success)]()
...
一个 **编排器（orchestrator）skill + 全套依赖合集**：编排器本身不解题、不写作，而是按
**G0→G6 七道门**，把每个阶段路由给该领域最强的开源专科 skill，并全程叠加三层约束——
**中文/CUMCM 规范 · 高分论文范式 · 学术诚信红线**。本仓库已捆绑/接入全部 12 个所需 skill，
**克隆 + 一条命令 = 完整工作流就位**。

> *An orchestrator skill bundled with its full dependency set — one clone, one install
> script, and the end-to-end math-modeling pipeline (CUMCM-first, MCM/ICM adaptable) is ready.*
...
```bash
# 1. 克隆本仓库
git clone https://github.com/luxu0628/math-modeling-flow
cd math-modeling-flow

# 2. 一键安装全部 skill 到 ~/.claude/skills/
#    Windows：
...
> 安装脚本做两件事：把仓库内捆绑的 8 个 MIT skill 复制进 `~/.claude/skills/`；
> 再从原仓库现场拉取 2 个未声明许可的上游（本仓库不分发其代码，见
> [THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md)）。需要 git 与网络。
> 不想用脚本？手动安装见 [_装配说明.md](_装配说明.md)。
...
| 装一堆 skill 版本对不上 | **本仓库即合集**：捆绑+锁定一套经过实测的组合 |

## 🧩 合集清单：一根脊柱 + 四类器官

| 角色 | skill | 上游 | 集成方式 |
|---|---|---|---|
| 🦴 脊柱·总指挥 | `math-modeling-contest` | [xuec699-sudo/math-modeling-skills](https://github.com/xuec699-sudo/math-modeling-skills) | 安装时拉取* |
| 📚 解题/写作内容库 | `math-modeling-solver` · `math-modeling-paper` | [Lupynow/math-modeling-skills](https://github.com/Lupynow/math-modeling-skills) | 安装时拉取* |
| 🎨 投稿级出图/学术 | `nature-figure` · `nature-writing` · `nature-polishing` · `nature-academic-search` | [Yuan1z0825/nature-skills](https://github.com/Yuan1z0825/nature-skills) | ✅ 已捆绑（MIT） |
| ⚙️ 过程纪律 | `brainstorming` · `systematic-debugging` · `verification-before-completion` | [obra/superpowers](https://github.com/obra/superpowers) | ✅ 已捆绑（MIT） |
| 🏛️ 治理与交付 | `chinese-thesis-workbench` | [ZyhSechub/chinese-thesis-workbench-skill](https://github.com/ZyhSechub/chinese-thesis-workbench-skill) | ✅ 已捆绑（MIT） |

\* 上游未声明开源许可，本仓库不分发其代码，由安装脚本从原仓库获取。详见 [THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md)。
...
├── skills/                    # 捆绑的 8 个 MIT 上游 skill（各含上游 LICENSE）
│   ├── nature-figure/         #   出图（assets 演示图库已精简，见 NOTICES）
│   ├── nature-writing/  nature-polishing/  nature-academic-search/
│   ├── brainstorming/  systematic-debugging/  verification-before-completion/
│   └── chinese-thesis-workbench/
├── references/
│   └── 高分论文范式.md         # 13 篇真实获奖论文提炼的写作/检验规则
├── install.ps1 / install.sh   # 一键安装（捆绑复制 + 上游拉取）
├── _装配说明.md                # 手动安装与路由说明
...
本编排器站在以下开源项目的肩膀上，只做"编排 + CUMCM 适配 + 合集封装"，再次感谢各位作者：
...
- 本仓库自有内容（编排器、范式文档、脚本）：**MIT**（见 [LICENSE](LICENSE)）
- 捆绑的第三方 skill：各自 MIT，原文保留在各自文件夹
- 安装时拉取的上游：版权归原作者，详见 [THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md)
